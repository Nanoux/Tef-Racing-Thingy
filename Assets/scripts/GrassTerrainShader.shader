...
#pragma target 4.0
	...
	void geom(point v2g IN[1], inout TriangleStream<g2f> triStream)
	{
		...
		//float3 faceNormal = perpendicularAngle;
		...
		float cos30 = sin60;
		float cos60 = sin30;
		int posoffset = tex2Dlod (_MainTex, float4(IN[0].pos.x,IN[0].pos.z,0,0)).g;

		g2f OUT;

		// Quad 1

		OUT.pos = UnityObjectToClipPos(v0 + perpendicularAngle * 0.5 * _GrassHeight) + posoffset;
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(1, 0);
		triStream.Append(OUT);

		OUT.pos = UnityObjectToClipPos(v1 + perpendicularAngle * 0.5 * _GrassHeight) + posoffset;
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(1, 1);
		triStream.Append(OUT);

		OUT.pos = UnityObjectToClipPos(v0) + posoffset;
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(0.5, 0);
		triStream.Append(OUT);

		OUT.pos = UnityObjectToClipPos(v1) + posoffset;
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(0.5, 1);
		triStream.Append(OUT);
		...
		OUT.pos = UnityObjectToClipPos(v0 + float3(sin60, 0, -cos60) * 0.5 * _GrassHeight) + posoffset;
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(1, 0);
		triStream.Append(OUT);

		OUT.pos = UnityObjectToClipPos(v1 + float3(sin60, 0, -cos60)* 0.5 * _GrassHeight) + posoffset;
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(1, 1);
		triStream.Append(OUT);
