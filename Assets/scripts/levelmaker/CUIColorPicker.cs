...
public class CUIColorPicker : MonoBehaviour    //https://www.youtube.com/watch?v=Pmm9emSL6Pc source!
{
	...
	public Color resultCol;
	public GameObject R, G, B, HEX;
    ...
    private void Setup( Color inputColor )
    {
        ...
        Action applySaturationValue = () => {
			...
			resultCol = c0 + c1 + c2 + c3;
			R.GetComponent<InputField>().text = "" + (int)Mathf.Round(resultCol.r * 256);
			G.GetComponent<InputField>().text = "" + (int)Mathf.Round(resultCol.g * 256);
			B.GetComponent<InputField>().text = "" + (int)Mathf.Round(resultCol.b * 256);
			HEX.GetComponent<InputField>().text = ColorToHex(resultCol);//hi
			//HEX.GetComponent<InputField>().text = "" + ColorUtility.TryParseHtmlString
            var resImg = result.GetComponent<Image>();
			resImg.color = resultCol;
			if ( _color != resultCol ) {
                if ( _onValueChange != null ) {
					_onValueChange( resultCol );
                }
				_color = resultCol;
            }
        };
	...
	public void red(string a){
		float blue;
		if (float.TryParse(a,out blue)){
			Color = new Color(blue / 256, Color.g,Color.b);
		}
	}public void green(string a){
		float blue;
		if (float.TryParse(a,out blue)){
			Color = new Color(Color.r, blue / 256,Color.b);
		}
	}public void blue(string a){
		float blue;
		if (float.TryParse(a,out blue)){
			Color = new Color(Color.r, Color.g,blue / 256);
		}
	}public void HexEdit(string a){
		Color = HexToColor(a);
	}
